...
namespace http
{
  ...
  class CFileSystem : public AMiddleware
  {
    ...
    {
      log::info << "http::CFileSystem::CFileSystem()" << log::endl;
...
*/
    ...
    {
      log::info << "http::CFileSystem::use(http::CApp)" << log::endl;
      ...
      app.mServer.on("request", [this](http::CRequest& req, http::CResponse& res) {
        ...
        if(req.verb() == EVerb::GET)
        {
          ...
          // @todo Find a better way to check if the request is a file
