...
namespace http
{
  class CRouter // @todo CRouter should be a AMiddleware!
  {
    ...
    {
      log::debug << "http::CRouter::CRouter(server)" << log::endl;
    }

    ~CRouter()
    {
      log::debug << "http::CRouter::~CRouter()" << log::endl;
    }

    protected:
    void init()
    {
      log::debug << "http::CRouter::init()" << log::endl;
      ...
      mServer.on("request", [this](http::CRequest& req, http::CResponse& res) {
        ...
        for(auto it = mRoutes.begin(); it != mRoutes.end(); ++it)
        {
          ...
          if(verb == req.verb() || verb == EVerb::ALL) // verb or all
          {
            ...
            {
              log::info << "> [match] " << http::getVerb(verb) << " " << path << log::endl;
              call(req, res);
              break;            // only one route
            }
          }

          log::info << "> [skip]  " << http::getVerb(verb) << " " << path << log::endl;
        }
      });
    }
    ...
    CRouter& match(EVerb verb, const path_t& path, std::function<void(CRequest&, CResponse&)>&& callback)
    {
      ...
      log::info << "> Registring route: " << path << log::endl;
      return *this;
    }
  };
}
